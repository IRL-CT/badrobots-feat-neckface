...
class ResNet34(nn.Module):
    def __init__(self, num_classes, activation='relu', num_batch = 512):
        ...
        # Add new fully connected layers with batch normalization
        self.fc1 = nn.Linear(in_features, num_batch)
        self.bn1 = nn.BatchNorm1d(num_batch)
        self.fc2 = nn.Linear(num_batch, num_classes)
...
def train_wrapper(study_a_participants, study_a_data_path, study_a_splits, model_output, model_last_save, num_epochs, resume=0, resume_fold=0, config = None):
    ...
    def train(resume = 0, resume_fold = 0, config= None): #MODEL SAVE
        ...
        with wandb.init(config=config):
            ...
            wandb.log({"MODEL": "ResNet34 Batch"})
            ...
            for fold in range(len(train_folds)):
                ...
                classes = [0,1]
                model = ResNet34(num_classes=len(classes), activation=activation, num_batch=num_batch)
                if optimizer == 'adam':
...
def main():
    ...
    #CHANGE - added wandb and indentations for all inside main
        ...
        #MODEL SAVE
        last_model_save_dir = output_directory + "model_data/" + "resnet34_batch_30fps/"
