...
param_grid_1 = {'model': ['gMLP'], 'n_epoch': [5], 'dropout_LSTM_FCN': [nan], 'fc_dropout_LSTM_FCN': [nan], 'n_estimators': [nan], 'stride_train': [30], 'stride_eval': [30], 'lr': [0.0002], 'focal_loss': [False], 'interval_length': [5], 'context_length': [0], 'oversampling': [False], 'batch_size': [64], 'batch_tfms': [None], 'dataset_processing': [nan]}
...
model_pretrained = 'gMLP'
dataset_pretrained = 'openface'
    ...
    if i >= 0:
        if True:
            ...
            config = AttrDict(
                ...
                model_numbering = i,
                end_folder = '../../data/final_models/',
                model_pretrained = model_pretrained,
                dataset_pretrained = dataset_pretrained
            ...
            cross_validate_pretrained(val_fold_size=5, config=config,
                        group="pretrained", name=str(grid_config))
