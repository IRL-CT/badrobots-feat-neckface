...
def train(df):
    ...
    for epoch in range(config.epochs):
        ...
        metrics_val = {f'val_{k}': v for k, v in metrics_val.items()}

        wandb.log({
            'val_loss': val_loss / len(val_loader),
            'val_d_accuracy': val_accuracy,
            **metrics_val
        })
...
def main():
    ...
    sweep_config = {
        'method': 'random',
        'name': 'transformer_neckface_openface',
        'parameters': {
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project="transformer_neckface_openface")
